...
namespace Glow {
	...
	bool init(const Glow_Init_Info& init_info) {
		...
		Entity e5555222 = Entity(glm::vec3(0.0f, 0.5f, 3.0f), glm::quat(1.0f, 0.0f, 0.0f, 0.0f), glm::vec3(1.0f), "918/scene.gltf", false);
		scene.include(e5555222);

		//Entity e55552222 = Entity(glm::vec3(0.0f), glm::quat(1.0f, 0.0f, 0.0f, 0.0f), glm::vec3(1.0f), "room/scene.gltf", false);
		//scene.include(e55552222);
			
		//Entity e5555(vec3(0.0f), glm::quat(1.0f, 0.0f, 0.0f, 0.0f), vec3(1.0f), "Sponza/glTF/Sponza.gltf", false);
		//scene.include(e5555);
		
		//Entity e55552222 = Entity(glm::vec3(0.0f, 20.5f, 3.0f), glm::quat(1.0f, 0.0f, 0.0f, 0.0f), glm::vec3(1.0f), "911-2/scene.gltf", false);
		//scene.include(e55552222);

		//Entity gsdgfsd = Entity(glm::vec3(0.0f, 20.5f, 3.0f), glm::quat(1.0f, 0.0f, 0.0f, 0.0f), glm::vec3(1.0f), "rainbow_road/scene.gltf", false);
	...
	void incandesce() {
		...
		while (window.open()) {
			...
			if (!editor_mode) {
				...
				Physics::update(); // todo default 1/60 delta time
			} }
			...
			renderer.begin_frame(scene, player_view, player_inv_view, player_proj, active_view, active_inv_proj, aspect_ratio, player.camera.zoom);
			...
			{ PROFILE_SCOPE_COLOR("particles", legit::Colors::peterRiver);
			...
			{ PROFILE_SCOPE_COLOR("debug", legit::Colors::sunFlower);
			...
			gpuGraph.RenderTimings(300, 200, 225, 0, 1.0f / 60.0f);
